...
Актуальная сборка: **web-87** (в UI: **Главная** → внизу «Сборка: web-XX»; номер подставляется из `GAZPROM_WEB_BUILD` в `app.js` через `syncAppBuildLabel()`). См. §3.8 — фильтры и сортировка истории; §3.9 — баннер статуса данных.
...
- **Нижний бар:** `fixed`, `bottom: 0`, SVG-иконки (наследие web-78), сдвиг вниз `--gazprom-nav-shift-down` (web-62).
...
### 3.9. Баннер статуса данных (web-86, web-87)
...
- На телефоне (`body.gazprom-mobile-shell`): при peek — **fixed** оверлей сверху (`z-index: 1200`, safe-area), уезжает вверх `transform`, не сдвигает layout `.app`.
- Для пустой / «свежей» базы — `resetDataStatusPeek`, баннер остаётся статичным (без автоскрытия).
...
| web-87 | **Баннер данных:** доработка peek (таймеры 3,5 с / 0,75 с, `aria-live`, fixed-оверлей на телефоне); актуальная сборка |
...
1. **Главная** → внизу по центру «Сборка: web-87» (после обновления; подпись из `app.js`).
...
13. **Баннер данных** — после синхронизации/импорта кратко «✓ актов · орг · фото», затем плавно исчезает; на телефоне — сверху поверх контента, не ломает скролл.
...
- [ ] Навигация v4 (FAB desktop + центр bottom bar) — макет есть, в прод не включено после web-81
...
_Обновлено под **web-87**. При следующем релизе — синхронизировать номер сборки, §7 и чеклист §8._
