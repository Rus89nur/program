...
Актуальная сборка: **web-222** (в UI: **Главная** → внизу «Сборка: web-XX»; номер из `GAZPROM_ASSET_V` в `index.html`, дублируется в `app.js` → `GAZPROM_WEB_BUILD`). См. §3.3 — шаг «Нарушения»; **§3.11** — модалка редактирования нарушения; **§3.14** — мастер справки; §3.6 — устранение; **§3.12** — график проверок; **§3.13** — одноколоночная сетка; §3.8 — история; §3.10 — нижняя навигация.
...
3. **Обновить этот файл** — строка сборки в шапке документа, §3.11, §3.14, таблица §7, §8.

---

## 8. Чеклист проверки на телефоне

1. **Главная** → внизу по центру актуальная «Сборка: web-222» (или новее); метка **не прыгает** при обновлении страницы; на **мастере** подписи сборки **нет**.
...
_Обновлено под **web-222**. При следующем релизе — синхронизировать `GAZPROM_ASSET_V`, §3.11, §3.12–3.14, §7, §8._
