...
Актуальная сборка: **web-94** (в UI: **Главная** → внизу «Сборка: web-XX»; номер подставляется из `GAZPROM_WEB_BUILD` в `app.js` через `syncAppBuildLabel()`). См. §3.8 — фильтры и сортировка истории; §3.9 — баннер статуса данных; §3.10 — нижняя навигация.
...
1. **Главная** → внизу по центру «Сборка: web-94» (после обновления; подпись из `app.js`).
