...
Актуальная сборка: **web-81** (в UI: **Главная** → внизу «Сборка: web-XX»; номер подставляется из `GAZPROM_WEB_BUILD` в `app.js`). См. §3.8 — фильтры и сортировка истории.
...
### 3.8. Экран «История» (web-75, web-77, web-80, web-81)
...
- Панель фильтров — **колонка** (поиск сверху, затем **одна** горизонтальная полоса pills `.toolbar-filters--pills`: `flex-wrap: nowrap`, `overflow-x: auto`, `touch-action: pan-x`, `padding-right` для прокрутки до «Полные» / «Черновики»). В `mobile-overlay.js` — `.toolbar-filters--pills` в `allowHorizontalScroll`.
- Блок **«Сортировка»** (`.history-list-toolbar`) — **одна строка**, горизонтальный скролл при нехватке места; кнопка организации подписана **«Орг.»** (`aria-label="Организация"`), компактные чипы на телефоне. `.history-list-toolbar` тоже в `allowHorizontalScroll`.
...
| web-81 | **История:** pills и «Сортировка» — одна строка, полный горизонтальный свайп (без залипания на «Сокращённые»); сортировка «Орг.»; `allowHorizontalScroll` + `touch-action: pan-x` |
...
1. **Главная** → внизу по центру «Сборка: web-81» (после обновления страницы).
...
- [x] История: горизонтальный скролл фильтров/сортировки, «Орг.» — web-81
...
_Обновлено под web-81. При следующем релизе — синхронизировать номер сборки и §7._
