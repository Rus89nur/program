...
Документ для продолжения работы **без контекста чата**. Актуальная сборка на момент последнего обновления файла: **web-60** (проверьте в UI: Настройки → «Сборка: web-XX»).
...
- `app.css` landscape: шапка `flex-direction: row`; `--gazprom-nav-block` ~36px; bottom-nav без подписей; `#screen-settings .settings-grid` — `minmax(108px, 1fr)`, у плиток скрыт подзаголовок `p`.
...
1. Настройки → сборка совпадает с ожидаемой.
