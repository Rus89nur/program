...
| web-82 | **История:** рабочий горизонтальный свайп pills/сортировки — `pan-x` на кнопках, расширенные селекторы в `mobile-overlay.js`, capture-shield + ручной `scrollLeft`, overflow оболочек |
| web-85 | **История:** кнопки сортировки крупнее для tap/readability — 13px, padding 10×12px, min-height 44px на телефоне (горизонтальный ряд без изменений) |
...
11. **История** — карточки на всю ширину как на Устранении; иконка слева; в шапке номер, организация (`shortTitle`), «Проверка: дата»; в подвале объект и тип акта; справа красная **×**; pills в **одну строку** — свайп до «Черновики»; «Сортировка» в одну строку, **Орг.**, кнопки сортировки удобного размера (не 11px); нет «Экспорт списка».
