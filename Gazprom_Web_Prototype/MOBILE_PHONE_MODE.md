...
Актуальная сборка: **web-114** (в UI: **Главная** → внизу «Сборка: web-XX»; номер из `GAZPROM_ASSET_V` в `index.html`, дублируется в `app.js` → `GAZPROM_WEB_BUILD`). См. §3.3 — шаг «Нарушения»; **§3.11** — модалка редактирования нарушения; §3.8 — история; §3.10 — нижняя навигация.
...
### 3.1. Сборка приложения и деплой (web-63, web-69, web-112–114)
...
1. **Главная** → внизу по центру актуальная «Сборка: web-114» (или новее); на **мастере** подписи сборки **нет**.
...
8. **Модалка нарушения** — на **весь экран**; выделение текста в полях (ручки двигаются); закрытие: ×, «Отмена», тап по затемнению; футер: **Удалить | Сохранить | Отмена** в одну строку, равная ширина.
...
_Обновлено под **web-114**. При следующем релизе — синхронизировать `GAZPROM_ASSET_V`, §3.11, §7, §8._
