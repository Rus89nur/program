...
Актуальная сборка: **web-122** (в UI: **Главная** → внизу «Сборка: web-XX»; номер из `GAZPROM_ASSET_V` в `index.html`, дублируется в `app.js` → `GAZPROM_WEB_BUILD`). См. §3.3 — шаг «Нарушения»; **§3.11** — модалка редактирования нарушения; §3.6 — устранение; **§3.12** — график проверок; **§3.13** — одноколоночная сетка; §3.8 — история; §3.10 — нижняя навигация.
