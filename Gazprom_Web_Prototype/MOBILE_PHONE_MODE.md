...
Актуальная сборка: **web-76** (в UI: **Главная** → внизу «Сборка: web-XX»; номер подставляется из `GAZPROM_WEB_BUILD` в `app.js`).
...
### 3.8. Экран «История» (web-75)

- Строка списка: подзаголовок — **тип акта и число нарушений** (без названия объекта).
- Организация — **`AktSearch.getOrgTitle`** (`shortTitle`, иначе `title`), отдельная строка `.history-list-item__org` с переносом слов.
- На телефоне: карточка строки **столбиком** (организация на всю ширину под заголовком); панель фильтров — **колонка** (поиск, затем pills); блок «Сортировка» не перекрывается toolbar.
...
1. **Главная** → внизу по центру «Сборка: web-76» (после обновления страницы).
...
11. **История** — в строке видно краткое название организации; pills года и блок «Сортировка» полностью видны и нажимаются; нет кнопки «Экспорт списка».
...
- [x] История: читаемые строки, фильтры — web-75
...
_Обновлено под web-76. При следующем релизе — синхронизировать номер сборки и §7._
