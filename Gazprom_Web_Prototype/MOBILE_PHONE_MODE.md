...
Актуальная сборка: **web-82** (в UI: **Главная** → внизу «Сборка: web-XX»; номер подставляется из `GAZPROM_WEB_BUILD` в `app.js`). См. §3.8 — фильтры и сортировка истории.
...
### 3.8. Экран «История» (web-75, web-77, web-80, web-81, web-82)
...
- Блок **«Сортировка»** (`.history-list-toolbar`) — **одна строка**, горизонтальный скролл при нехватке места; кнопка организации подписана **«Орг.»** (`aria-label="Организация"`), компактные чипы на телефоне.
...
1. **Главная** → внизу по центру «Сборка: web-82» (после обновления страницы).
...
11. **История** — карточки на всю ширину как на Устранении; иконка слева; в шапке номер, организация (`shortTitle`), «Проверка: дата»; в подвале объект и тип акта; справа красная **×**; pills в **одну строку** — свайп до «Черновики»; «Сортировка» в одну строку, **Орг.**; нет «Экспорт списка».
...
_Обновлено под web-82. При следующем релизе — синхронизировать номер сборки и §7._
