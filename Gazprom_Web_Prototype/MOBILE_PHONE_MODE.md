...
Актуальная сборка: **web-77** (в UI: **Главная** → внизу «Сборка: web-XX»; номер подставляется из `GAZPROM_WEB_BUILD` в `app.js`).
...
### 3.8. Экран «История» (web-75, web-77)

- Разметка карточки как на **Устранении**: иконка слева; **`history-list-item__content`** — шапка (номер акта, `shortTitle` организации через `AktSearch.getOrgTitle`, кнопка **🗑** справа) и **`history-list-item__foot`** (дата, тип акта).
...
- Подзаголовок — **только тип акта** («Полный акт» / «Сокращённый акт»), **без** числа нарушений.
- Бейдж «Завершён» / «Черновик» убран; вместо статуса справа в шапке — **кнопка удаления** (в корзину); для черновиков — компактная метка «Черновик» в подвале карточки.
...
1. **Главная** → внизу по центру «Сборка: web-77» (после обновления страницы).
...
11. **История** — карточки: иконка слева, организация (`shortTitle`) под номером акта, дата и тип акта в подвале; справа в шапке кнопка 🗑 (без бейджа «Завершён»); pills года и блок «Сортировка» полностью видны и нажимаются; нет кнопки «Экспорт списка».
...
_Обновлено под web-77. При следующем релизе — синхронизировать номер сборки и §7._
