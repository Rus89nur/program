...
Актуальная сборка: **web-72** (в UI: **Главная** → внизу «Сборка: web-XX»; номер подставляется из `GAZPROM_WEB_BUILD` в `app.js`).
...
- Фильтры организаций — **вертикальный** список в `.pred-filter-row`: колонка, `max-height: min(160px, 30vh)`, `overflow-y: auto`, `overscroll-behavior: contain` (внутренний скролл, без захвата жеста у `.main`).
...
1. **Главная** → внизу по центру «Сборка: web-72» (после обновления страницы).
...
5. **Мастер шаг 5** — даты столбиком, чипы представителей на всю ширину, без горизонтального сдвига; фильтры организаций — вертикальный скролл, все организации доступны.
...
_Обновлено под web-72. При следующем релизе — синхронизировать номер сборки и §7._
