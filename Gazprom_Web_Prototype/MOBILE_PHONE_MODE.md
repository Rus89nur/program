...
Актуальная сборка: **web-80** (в UI: **Главная** → внизу «Сборка: web-XX»; номер подставляется из `GAZPROM_WEB_BUILD` в `app.js`).
...
### 3.8. Экран «История» (web-75, web-77, web-80)
...
1. **Главная** → внизу по центру «Сборка: web-80» (после обновления страницы).
...
11. **История** — карточки на всю ширину как на Устранении; иконка слева; в шапке номер, организация (`shortTitle`), «Проверка: дата»; в подвале объект и тип акта; справа красная **×**; pills и «Сортировка» видны; нет «Экспорт списка».
...
_Обновлено под web-80. При следующем релизе — синхронизировать номер сборки и §7._
