...
Актуальная сборка: **web-85** (в UI: **Главная** → внизу «Сборка: web-XX»; номер подставляется из `GAZPROM_WEB_BUILD` в `app.js`). См. §3.8 — фильтры и сортировка истории.
...
| `js/ui-bindings.js` | `renderHistory`, `renderDataStatus` — полный и компактный текст баннера |
...
### 3.8. Экран «История» (web-75, web-77, web-80, web-81, web-82, web-85)
...
| web-75 | История: краткая организация в строке списка, без объекта в подзаголовке; mobile — столбик + читаемый перенос; toolbar фильтров/сортировки без наложения; удалён «Экспорт списка» |
| web-76 | **Устранение:** сетка карточек на телефоне, объект на всю ширину, читаемые переносы; `shortTitle` в шапке карточки |
| web-77 | **История:** карточки как на Устранении — сетка на телефоне, `shortTitle` организации, дата и тип акта в подвале; кнопка удаления вместо бейджа «Завершён»; без числа нарушений в подзаголовке |
| web-79 | **История:** в подвале — название объекта и «Проверка: дата» под ним; удаление — красная **×** (десктоп и телефон) |
| web-80 | **История:** карточки по ширине/отступам как Устранение; «Проверка: дата» в шапке (`__head`), как «Срок:» на Устранении; список без лишних боковых padding |
| web-81 | **История:** pills и «Сортировка» — одна строка, сортировка «Орг.»; `allowHorizontalScroll` + `touch-action: pan-x` (на iPhone свайп по pills всё ещё блокировался — см. §3.8) |
| web-82 | **История:** рабочий горизонтальный свайп pills/сортировки — `pan-x` на кнопках, расширенные селекторы в `mobile-overlay.js`, capture-shield (`stopPropagation`), overflow оболочек |
| web-85 | **История:** кнопки сортировки крупнее (13px, padding 10×12px, min-height 44px); **плавный** горизонтальный скролл pills/сортировки — убран ручной `scrollLeft`, нативная инерция iOS |
...
1. **Главная** → внизу по центру «Сборка: web-85» (после обновления страницы).
...
11. **История** — карточки на всю ширину как на Устранении; иконка слева; в шапке номер, организация (`shortTitle`), «Проверка: дата»; в подвале объект и тип акта; справа красная **×**; pills в **одну строку** — **плавный** свайп с инерцией до «Черновики»; «Сортировка» в одну строку, **Орг.**, кнопки сортировки удобного размера (не 11px); нет «Экспорт списка».
12. **Устранение** — карточки: кольцо слева, бейдж справа в шапке; объект/описание на всю ширину, без рваных переносов; организация — краткое имя.
...
- [x] История: карточки актов на телефоне — web-77
- [x] История: объект и дата проверки в подвале, красная × — web-79
- [x] История: ширина карточек как Устранение, дата в шапке — web-80
- [x] История: горизонтальный скролл фильтров/сортировки, «Орг.» — web-81 (частично)
- [x] История: свайп pills до «Черновики» на iPhone — web-82
- [x] История: крупные кнопки сортировки (№, Дата, Орг., Наруш.) — web-85
- [ ] Мастер шаги 2–3 (организация, объект)
- [x] Устранение — карточки актов на телефоне — web-76
...
_Обновлено под web-85. При следующем релизе — синхронизировать номер сборки и §7._
