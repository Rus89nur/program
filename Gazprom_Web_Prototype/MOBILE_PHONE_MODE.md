...
Актуальная сборка: **web-70** (в UI: **Главная** → внизу «Сборка: web-XX»; номер подставляется из `GAZPROM_WEB_BUILD` в `app.js`).
...
### 3.6. Кнопки «Назад» / «Далее» (web-66, web-68, web-70)

- На телефоне: **одна строка**, `justify-content: center`, gap 10px.
- Кнопки уже половины экрана: `flex: 0 1 auto`, `max-width: 46%`, padding 10×8px, font 14px, `min-height: 48px`, текст с переносом.
- Подпись последнего шага: **«💾 Сохранить черновик»** (полная, как на десктопе).
- web-67 временно уменьшал кнопки (12px) — **откат в web-68**; web-66 — половина на половину (`flex: 1`) до web-70.
...
1. **Главная** → внизу по центру «Сборка: web-70» (после обновления страницы).
...
7. **Назад / Далее** — в одну строку по центру, уже половины экрана; на последнем шаге читаемо «💾 Сохранить черновик».
...
- [x] Кнопки мастера в строку — web-66 / web-68 / web-70 (уже)
...
_Обновлено под web-70. При следующем релизе — синхронизировать номер сборки и §7._
