...
Актуальная сборка: **web-90** (в UI: **Главная** → внизу «Сборка: web-XX»; номер подставляется из `GAZPROM_WEB_BUILD` в `app.js` через `syncAppBuildLabel()`). См. §3.8 — фильтры и сортировка истории; §3.9 — баннер статуса данных; §3.10 — нижняя навигация.
...
### 3.10. Нижняя навигация — outline + подчёркивание (web-88, web-89)
...
1. **Главная** → внизу по центру «Сборка: web-90» (после обновления; подпись из `app.js`).
...
- [x] Устранение: диалог продления срока поверх списка нарушений — web-90
...
_Обновлено под **web-90**. При следующем релизе — синхронизировать номер сборки, §7 и чеклист §8._
