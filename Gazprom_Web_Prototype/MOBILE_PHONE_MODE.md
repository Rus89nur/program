...
Актуальная сборка: **web-73** (в UI: **Главная** → внизу «Сборка: web-XX»; номер подставляется из `GAZPROM_WEB_BUILD` в `app.js`).
...
- Фильтры организаций — **горизонтальная** полоса в `.pred-filter-row`: `flex-wrap: nowrap`, `overflow-x: auto`, `overflow-y: hidden`, `-webkit-overflow-scrolling: touch`, `overscroll-behavior-x: contain`. В `mobile-overlay.js` — `.pred-filter-row` в `allowHorizontalScroll` (как `.toolbar-filters--pills`), чтобы на iPhone работал свайп по фильтрам.

### 3.5. Сводка акта в мастере (web-70)
...
- Содержимое строк — в `.summary-panel__body`; обновление по-прежнему в `updateSummary()`.

### 3.6. Кнопки «Назад» / «Далее» (web-66, web-68, web-71)
...
| web-72 | Выводы: фильтры организаций — вертикальный скролл в `.pred-filter-row` (колонка, max-height 160px/30vh); чипы фильтра на всю ширину |
| web-73 | Выводы: откат web-72 — фильтры организаций снова **горизонтальный** скролл (как web-67); `.pred-filter-row` в `allowHorizontalScroll` |
...
1. **Главная** → внизу по центру «Сборка: web-73» (после обновления страницы).
...
5. **Мастер шаг 5** — даты столбиком, чипы представителей на всю ширину, без горизонтального сдвига страницы; фильтры организаций — **горизонтальный** свайп/скролл по ряду чипов.
6. **Сводка акта** — свёрнута по умолчанию; по нажатию на заголовок раскрываются строки.
7. **Назад / Далее** — в одну строку, равная ширина по краям (`space-between`); на последнем шаге читаемо «💾 Сохранить черновик».
8. Модалка нарушения — по центру, ~90% ширины.
9. Поворот landscape ↔ portrait — вёрстка не «ломается»; в landscape баннер — одна строка.
10. Нижний бар — удобная высота, не перекрывает контент.
...
- [x] Кнопки мастера в строку — web-66 / web-68 / web-71 (равная ширина)
- [x] Сводка акта сворачиваемая — web-70
...
_Обновлено под web-73. При следующем релизе — синхронизировать номер сборки и §7._
