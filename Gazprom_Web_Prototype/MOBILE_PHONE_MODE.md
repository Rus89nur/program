...
Актуальная сборка: **web-79** (в UI: **Главная** → внизу «Сборка: web-XX»; номер подставляется из `GAZPROM_WEB_BUILD` в `app.js`).
...
### 3.8. Экран «История» (web-75, web-77, web-79)

- Разметка карточки как на **Устранении**: иконка слева; **`history-list-item__content`** — шапка (номер акта, `shortTitle` организации через `AktSearch.getOrgTitle`, кнопка **×** красного цвета справа) и **`history-list-item__foot`** (объект, дата проверки, тип акта).
- На телефоне (`body.gazprom-mobile-shell`): карточка — **CSS Grid** (`icon` | `header`, затем `foot` на **всю ширину**); `display: contents` у `__content`.
- Подвал: **`history-list-item__object`** — название объекта из `(akt.objectsCheck || [])[0].title` (fallback `subTitle`), как на Устранении; ниже **`history-list-item__date`** — «Проверка: DD.MM.YYYY» (`akt.date`, `AktUtils.formatDateShort`); затем тип акта.
...
- Панель фильтров — **колонка** (поиск, затем pills); блок «Сортировка» не перекрывается toolbar.
...
1. **Главная** → внизу по центру «Сборка: web-79» (после обновления страницы).
...
11. **История** — карточки: иконка слева, организация (`shortTitle`) под номером акта; в подвале объект, «Проверка: дата», тип акта; справа в шапке красная **×** (без бейджа «Завершён»); pills года и блок «Сортировка» полностью видны и нажимаются; нет кнопки «Экспорт списка».
...
_Обновлено под web-79. При следующем релизе — синхронизировать номер сборки и §7._
