...
class SettingsPredstavViewController: UIViewController {
    ...
    @objc private func addNew() {
        let alert = UIAlertController(title: "Добавление нового представителя", message: "Введите ФИО и Должность", preferredStyle: .alert)
        alert.addTextField()
        alert.addTextField()
        ...
        let save = UIAlertAction(title: "Добавить", style: .default) { [weak self] _ in
            ...
            if let text = alert.textFields?.first?.text, let subTitle = alert.textFields?[1].text {
                let newObj = PredstavitelyComission(fio: text, jobTitle: subTitle)
...
extension SettingsPredstavViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            model.predstavitelyArray.remove(at: indexPath.row)
            DataFlowPredstavitely.saveArr(arr: model.predstavitelyArray)
    ...
    private func showEditAlert(for indexPath: IndexPath) {
        ...
        let editVC = UniversalEditViewController(editType: editType) { [weak self] newFio, newJobTitle in
            ...
            self.model.updatePredstavitely(at: indexPath.row, newFio: newFio, newJobTitle: jobTitle) { success in
